    ...
    <a-table :data-source="data" :columns="columns" :scroll="{ x: 800 }" style="margin-top: 16px">
      ...
      <template slot="operation" slot-scope="text, record">
        <a-popconfirm
                v-if="data.length"
                title="确认要退选这门课吗?"
                @confirm="() => untakeCourses(record)"
        >
          <a href="javascript:;">退选</a>
        </a-popconfirm>
      </template>
    </a-table>
    <p>
    ...
    export default {
        ...
        methods: {
            fetchCoursesData() {
                ...
                axios({
                    method: 'get',
                    url: baseUrl + '/api/student/enrolled_courses',
                    headers: {
            ...
            untakeCourses(target) {
                let record = {
                    sid: getCookie("uid"),
                    cid: target.cid,
                };
                let contextVue = this;
                if (!record || !target.cid || !record.sid) {
                    return
                }
                axios({
                    method: 'delete',
                    ...
                    params: {
                        query: `cid = '${record.cid}' AND sid = '${record.sid}'`
                    }
                }).then(function (response) {
                    if (response.data && response.data.success) {
                        contextVue.$message.success("退选成功");
                        contextVue.fetchCoursesData();
                    } else if (response.data && response.data.error) {
                        contextVue.$message.error("退选失败: " + response.data.error)
                    } else {
                        contextVue.$message.error("退选失败，发生了未知错误。")
                    }
                });
            },
        ...
        mounted() {
            this.timer = setInterval(this.fetchCoursesData, 2000);
        },
        beforeDestroy() {
